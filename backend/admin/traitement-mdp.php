...
$sql = "SELECT mot_de_passe FROM enseignant WHERE id_enseignant = ?";
...
$sql = "UPDATE enseignant SET mot_de_passe = ? WHERE id_enseignant = ?";
$stmt = mysqli_prepare($connexion, $sql);
mysqli_stmt_bind_param($stmt, "si", $mdp_nouveau, $id_enseignant);
mysqli_stmt_execute($stmt);
